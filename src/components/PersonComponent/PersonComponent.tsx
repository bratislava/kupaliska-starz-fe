...
const PersonComponent = ({
  ...
  const showRemoveIcon =
    (mode === PeopleListMode.Profile || mode === PeopleListMode.OrderPageDisplay) && !removeDisabled
  const showCheckmarkIcon = mode === PeopleListMode.OrderPageSelection && isSelected
  ...
  const showGreenOverlay = mode === PeopleListMode.OrderPageSelection && isSelected
      ...
      >
        ...
        {showRightCorner && (
          <div className="absolute top-0 right-0 translate-x-1/2 -translate-y-1/2">
            ...
            {showCheckmarkIcon && <img src="/person-checkmark.svg" alt="" className="w-8 h-8" />}
          </div>
        )}
      </div>
      {showDescription && (
        <>
          ...
          {/* TODO: sklonovanie */}
          <div className="mt-3 text-center">{t('person-component.age', { age: person.age })}</div>
        </>
      )}
    </div>
  )

...
const TicketCardHomePage = ({ ticket, className = '', discount }: TicketCardProps) => {
  ...
  return (
    <div
      className={cx(
        ...
        {
          'hover:border-primary hover:bg-blueish cursor-pointer': !ticket.disabled,
        },
      )}
      onClick={handleClick}
    >
      <div className={`flex flex-col pt-6 px-3 xs:px-6 pb-4 rounded-t-lg justify-between`}>
        <Typography type="subtitle" fontWeight="bold">
          {ticket.name}
        </Typography>
        <p className="text-lg pt-4">{ticket.description}</p>
      </div>
      <div className={`flex flex-wrap px-3 xs:px-6 py-4 gap-1 items-center justify-between`}>
        <div

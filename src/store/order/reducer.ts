...
export const counterSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        checkDiscountCodeActions.fulfilled,
        (state, action: PayloadAction<DiscountCodeState>) => {
          state.status = 'idle'
...
export const selectCustomerInfoFormValues = (state: RootState) => state.order.customerInfo?.form
...
export const selectOrderPrice = (state: RootState) => {
  ...
  if (state.order.cart) {
    ...
    state.order.cart.ticket.childrenPrice &&
      (total += state.order.cart.childrenNumber * state.order.cart.ticket.childrenPrice)
  }
  if (
    state.order.discountCode &&
    state.order.discountCode.status === 'OK' &&
    state.order.discountCode.amount
  ) {
